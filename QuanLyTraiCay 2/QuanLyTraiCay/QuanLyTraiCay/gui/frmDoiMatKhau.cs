...
namespace QuanLyTraiCay.gui
{
    public partial class frmDoiMatKhau : Form
    {
        ...
        {
            btnDongY.Left += 2; // tốc độ trượt
            btnHuy.Left += 5;
            btnThoat.Left += 7;
            if(btnThoat.Left >= targetZ)
            {
                btnHuy.Left = targetZ;
                timer1.Stop();
            }
            if (btnHuy.Left >= targetY)
            {
                btnHuy.Left = targetY;
                timer1.Stop();
            }
            if (btnDongY.Left >= targetX)
            {
                btnDongY.Left = targetX;
                timer1.Stop();
            }
        }
        private void frmDoiMatKhau_Load(object sender, EventArgs e)
        {
            btnDongY.Left = -btnDongY.Width;
            btnThoat.Left = -btnThoat.Width;
