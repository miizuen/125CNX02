...
namespace QuanLyTraiCay.gui
{
    public partial class frmNhanVien : Form
    {
        ...
        private void dgvNhanVien_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            // Xử lý NgaySinh an toàn hơn
            object ngaySinhValue = dgvNhanVien.Rows[d].Cells[2].Value;
            if (ngaySinhValue != null)
            {
                try
                {
                    ...
                    DateTime ngaySinh;
                    if (DateTime.TryParse(ngaySinhValue.ToString(), out ngaySinh))
                    {
                        dtpNgaySinh.Value = ngaySinh;
                    }
                    else
                    {
                        // Nếu không parse được, để giá trị mặc định
