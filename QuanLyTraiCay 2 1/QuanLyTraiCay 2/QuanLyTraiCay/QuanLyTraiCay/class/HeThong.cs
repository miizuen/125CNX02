    ...
    class HeThong
    {
        ...
        void CapNhapTungBang(string tenBang)
        {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                string values = string.Join(",", table.Columns.Cast<DataColumn>().Select(c => "N'" + table.Rows[i][c.ColumnName].ToString().Trim() + "'"));
                string sql = $"INSERT INTO {tenBang} ({columnNames}) VALUES ({values})";
