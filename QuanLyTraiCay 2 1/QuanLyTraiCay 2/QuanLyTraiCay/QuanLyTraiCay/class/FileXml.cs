...
namespace QuanLyTraiCay.Class
{
    class FileXml
    {
        string Conn = @"Data Source=.; Initial Catalog = QuanLyTraiCay; Integrated Security=true";
        ...
        public void Xoa(string duongDan, string tenFileXML, string xoaTheoTruong, string giaTriTruong)
        {
            ...
            // Thử tìm với thẻ encoded trước (_x0027_NhanVien_x0027_)
            string encodedTag = "_x0027_" + tenFileXML + "_x0027_";
            XmlNode nodeCu = doc.SelectSingleNode("NewDataSet/" + encodedTag + "[" + xoaTheoTruong + "='" + giaTriTruong + "']");

            // Nếu không tìm thấy, thử tìm với thẻ thường (NhanVien)
            if (nodeCu == null)
            {
                nodeCu = doc.SelectSingleNode("NewDataSet/" + tenFileXML + "[" + xoaTheoTruong + "='" + giaTriTruong + "']");
            }
            ...
            {
                MessageBox.Show("Không tìm thấy dữ liệu để xóa với " + xoaTheoTruong + " = '" + giaTriTruong + "'");
            }
        }
