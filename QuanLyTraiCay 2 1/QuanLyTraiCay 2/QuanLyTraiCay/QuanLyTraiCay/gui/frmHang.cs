...
namespace QuanLyTraiCay.gui
{
    public partial class frmHang : Form
    {
        ...
        private void frmHang_Load(object sender, EventArgs e)
        {
            ...
            hienthiHang();
            btnHienThi.Left = -btnHienThi.Width;
            btnSua.Left = -btnSua.Width;// ẩn bên trái
            btnXoa.Left = -btnXoa.Width;
            btnThem.Left = -btnThem.Width;// ẩn bên trái
            btnReview.Left = -btnReview.Width;
            timer1.Start();
        }
        void LoadTable()
        {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            btnHienThi.Left += 9;
            btnReview.Left += 11;
            if (btnReview.Left >= targetB)
            {
                btnReview.Left = targetB;
                timer1.Stop();
            }
            if (btnHienThi.Left >= targetA)
            {
                btnHienThi.Left = targetA;
                timer1.Stop();
            }
            if (btnXoa.Left >= targetZ)
            {
                btnXoa.Left = targetZ;
                timer1.Stop();
            }
            if (btnSua.Left >= targetY)
            {
                btnSua.Left = targetY;
                timer1.Stop();
            }
